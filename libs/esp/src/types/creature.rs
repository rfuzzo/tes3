    ...
    fn insert_join_sql_record(&self, mod_name: &str, tx: &mut Transaction<'_>) -> rusqlite::Result<usize> {
        {
            ...
            for (idx, item_id) in &self.inventory {
                ...
                join.table_insert(&mut s, mod_name, &[&Null, &self.editor_id(), &Null])?;
            }
        }

        {
            ...
            for spell_id in &self.spells {
                ...
                join.table_insert(&mut s, mod_name, &[&Null, &Null, &self.editor_id(), &Null])?;
            }
        }

        {
            ...
            for dest in &self.travel_destinations {
                dest.table_insert(&mut s, mod_name, &[&self.editor_id(), &Null])?;
            }
        }

        {
            ...
            for package in &self.ai_packages {
                package.table_insert(&mut s, mod_name, &[&self.editor_id(), &Null])?;
            }
        }
